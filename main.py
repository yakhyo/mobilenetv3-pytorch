...
from nets.nn import mobilenet_v3_small
...
def load_data(traindir, valdir, args):
    ...
    dataset = torchvision.datasets.ImageFolder(
        ...
        transform=transforms.Compose([
            transforms.RandomResizedCrop(train_crop_size, interpolation=InterpolationMode.BILINEAR),
    ...
    dataset_test = torchvision.datasets.ImageFolder(
        valdir,
        transform=transforms.Compose([
            transforms.Resize(size=val_resize_size, interpolation=InterpolationMode.BILINEAR),
...
def main(args):
    ...
    if args.mixup_alpha > 0.0:
        mixup_transforms.append(T.RandomMixup(num_classes, p=1.0, alpha=args.mixup_alpha))
    if args.cutmix_alpha > 0.0:
        mixup_transforms.append(T.RandomCutmix(num_classes, p=1.0, alpha=args.cutmix_alpha))
    if mixup_transforms:
    ...
    if args.model_ema:
        ...
        model_ema = util.ExponentialMovingAverage(model_without_ddp, device=device, decay=1.0 - alpha)
...
if __name__ == "__main__":
    ...
    # torchrun --nproc_per_node = 2 train.py --epochs 300 --lr 0.045 --wd 0.00004 --lr-step-size 1 --lr-gamma 0.98
